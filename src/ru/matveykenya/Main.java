        ...
        try (SocketChannel socketChannel = SocketChannel.open()) {
            ...
            try (Scanner scanner = new Scanner(System.in)) {
                ...
                while (true) {
                    ...
                    думал проверить Буфер на наличие сообщение от сервера
                    но как-буд-то бы нет такой проверки или я на неверном пути - подскажите
                    В Слаке никто не отвечает - поэтому отправил код целиком на проверку
                     */
                    if (inputBuffer. ??????? ) {
                        //если без этого условия то будет работать обычно - ожидая ответа сервера sleep(5000)
                        int bytesCount = socketChannel.read(inputBuffer);
